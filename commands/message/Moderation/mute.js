...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if(!member){
      return message.reply({embeds: [new MessageEmbed().setColor(color).setDescription(`You didn't mentioned any member whom you want to mute.`)]})
    }
    const minutes = getValue(args[1]) / 60;
    if(minutes < 1){
      return message.reply({embeds: [new MessageEmbed().setColor(color).setDescription(`The duration of mute must not be less than **1m**.`)]})
    }
