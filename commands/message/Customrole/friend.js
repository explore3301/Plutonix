    ...
    run: async (client, message, args, prefix) => {
      ...
      color = client.main 
       ...
       // if(!prefix) prefix = Settings.bot.info.prefix;
        ...
        if(!args[0]){
            return message.channel.send({embeds : [new MessageEmbed().setColor(color).setDescription(`Usage : \`${prefix}\`friend <user>\``)]})
        }
        ...
        let gRole = await xd.get(`friend_${message.guild.id}`);
        if(!gRole || gRole == null){
            return message.channel.send({embeds : [new MessageEmbed().setColor(color).setDescription(`There is no **friend Role** set for **Custom Roles**`)]})
        }

        if(!message.guild.roles.cache.has(gRole)){
            await xd.set(`friend_${message.guild.id}`,null);
