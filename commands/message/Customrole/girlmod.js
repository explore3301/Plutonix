...
module.exports = {
    name : "girlmod",
    aliases : ["gurlmod","qtmod","qtzmod","womenmod"],
    category : "Customrole",
    run: async (client, message, args, prefix) => {
      ...
      color = client.main
        ...
        if(!args[0]){
            return message.channel.send({embeds : [new MessageEmbed().setColor(color).setDescription(`Usage : \`${prefix}\`girlmod <user>\``)]})
        }
        ...
        let gRole = await xd.get(`girlmod_${message.guild.id}`);
        if(!gRole || gRole == null){
            return message.channel.send({embeds : [new MessageEmbed().setColor(color).setDescription(`There is no **Girl Mod Role** set for **Custom Roles**`)]})
        }

        if(!message.guild.roles.cache.has(gRole)){
            await xd.set(`girlmod_${message.guild.id}`,null);
