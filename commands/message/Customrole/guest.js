...
module.exports = {
    ...
    run: async (client, message, args, prefix) => {
      ...
      color = client.main 
        ...
        if(!args[0]){
            return message.channel.send({embeds : [new MessageEmbed().setColor(color).setDescription(`Usage : \`${prefix}\`guest <user>\``)]})
        }
        ...
        let gRole = await xd.get(`guest_${message.guild.id}`);
        if(!gRole || gRole == null){
            return message.channel.send({embeds : [new MessageEmbed().setColor(color).setDescription(`There is no **guest Role** set for **Custom Roles**`)]})
        }

        if(!message.guild.roles.cache.has(gRole)){
            await xd.set(`guest_${message.guild.id}`,null);
