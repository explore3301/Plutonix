...
module.exports = {
    ...
    run: async (client, message, args, prefix) => {
      ...
      color = client.main
        ...
        if(!args[0]){
            return message.channel.send({embeds : [new MessageEmbed().setColor(color).setDescription(`Usage : \`${prefix}\`staff <user>\``)]})
        }
        ...
        let gRole = await xd.get(`staff_${message.guild.id}`);
        if(!gRole || gRole == null){
            return message.channel.send({embeds : [new MessageEmbed().setColor(color).setDescription(`There is no **staff Role** set for **Custom Roles**`)]})
        }

        if(!message.guild.roles.cache.has(gRole)){
            await xd.set(`staff_${message.guild.id}`,null);
