...
module.exports = {
    name : "girlvip",
    aliases : ["gurlvip","qtvip","qtzvip","womenvip"],
    category : "Customrole",
    run: async (client, message, args, prefix) => {
      ...
      color = client.main
        ...
        if(!args[0]){
            return message.channel.send({embeds : [new MessageEmbed().setColor(color).setDescription(`Usage : \`${prefix}\`girlvip <user>\``)]})
        }
        ...
        let gRole = await xd.get(`girlvip_${message.guild.id}`);
        if(!gRole || gRole == null){
            return message.channel.send({embeds : [new MessageEmbed().setColor(color).setDescription(`There is no **Girl Vip Role** set for **Custom Roles**`)]})
        }

        if(!message.guild.roles.cache.has(gRole)){
            await xd.set(`girlvip_${message.guild.id}`,null);
