...
module.exports = {
    name : "girladmin",
    aliases : ["gurladmin","qtadmin","qtzadmin","womenadmin"],
    category : "Customrole",
    run: async (client, message, args, prefix) => {
      ...
      color = client.main
        ...
        if(!args[0]){
            return message.channel.send({embeds : [new MessageEmbed().setColor(color).setDescription(`Usage : \`${prefix}\`girladmin <user>\``)]})
        }
        ...
        let gRole = await xd.get(`girladmin_${message.guild.id}`);
        if(!gRole || gRole == null){
            return message.channel.send({embeds : [new MessageEmbed().setColor(color).setDescription(`There is no **Girl Admin Role** set for **Custom Roles**`)]})
        }

        if(!message.guild.roles.cache.has(gRole)){
            await xd.set(`girladmin_${message.guild.id}`,null);
